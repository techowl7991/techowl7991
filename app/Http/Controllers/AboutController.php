...
class AboutController extends Controller
{
    ...
    public function add_user_account(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $data = [
                'firstname' => $request->eventstartdate,
                'lastname' => $request->eventenddate,
                'email' => $request->eventname,
                'password' => $request->eventpassword,

            ];

            $docref = self::$firestoreClient->collection('events')->document($uid)->collection('events_data')->add($data);
